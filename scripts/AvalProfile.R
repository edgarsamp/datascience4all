...
source("~/Relatorio2/Scripts/elattes.ls2df.R")

#Definir o local onde estão os arquivos json
#setwd("~/Documents/eLattes/Pacote e-Lattes/UnBPosGeral")
setwd("~/Relatorio2/CienciasdaComputacao/")
#setwd("~/Relatorio2/Computacao/")
#setwd("~/Relatorio2/Matematica/")
...
profile %>% 
  ...
  unlist() %>% table() %>% sort() %>% rev() %>% as.data.frame() %>% 
  ggplot(aes(x = ., y = Freq)) + geom_col(fill = "orange",alpha=0.9) + 
  geom_text(aes(label=comma(Freq)),size=3.5,vjust=-1) +
